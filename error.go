...
const (
	...
	// ErrURLNotFound is returned when the requested URL does not exist.
	ErrURLNotFound = "url_not_found"

	// Errors raised when interacting with exchange's API:

	// ErrInvalidResponse is returned when exchange API returned invalid response.
	// E.g. fields have bad formatting, response code differs from 200,
	// exchange answered with error message, etc.
	ErrInvalidResponse = "invalid_response"

	// ErrAPIBadRequest is returned when service could not understand the request
	// due to invalid syntax or missing a required parameter.
	ErrAPIBadRequest = "bad_request"
)
